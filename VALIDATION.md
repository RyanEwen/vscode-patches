...
[PR #336512](docs/patches/vscode-336512.md): 14 focused source tests passed, zero targeted TypeScript diagnostics, targeted lint and whitespace checks passed. Ten pre-install bundle tests passed, including pristine vendor-bundle reproduction; nine post-install tests passed per target with one unpatched-only check skipped. Staged exact rollback and installed hash checks passed. The patch is installed in Windows ARM64 and both Ubuntu ARM64 1.137.0 server locations. Processes were not restarted; live UI validation remains outstanding.
